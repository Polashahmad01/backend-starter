...
export const connectDatabase = async (): Promise<void> => {
  try {
    ...
    await mongoose.connect(`mongodb+srv://${config.database.mongoUserName}:${config.database.mongoPassword}@default.kp5ds1b.mongodb.net/${config.database.mongoDbName}?retryWrites=true&w=majority&appName=default`, options);
    ...
    mongoose.connection.on("reconnected", () => {
      console.log(colors.bgRed.white.bold("MongoDB reconnected"));
    });
